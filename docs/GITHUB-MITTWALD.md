...
- `Desktop installers`: unter Actions manuell startbar. Erstellt nach TypeScript- und Rust-Tests einen Windows-Installer und ein Linux-AppImage als herunterladbare Artefakte. Bei einem `v*`-Tag signiert und veröffentlicht der Workflow beide Dateien zusammen mit `latest.json` für den integrierten Updater als GitHub-Release.
